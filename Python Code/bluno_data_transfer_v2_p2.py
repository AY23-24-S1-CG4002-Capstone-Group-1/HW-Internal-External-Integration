...
BLUNO_GLOVE_TEST_MAC_ADDRESS = "D0:39:72:C8:56:46"
...
class BlunoGlove:
    ...
    def connectToBLEGlove(self):
        ...
        try:
            self.peripheral = Peripheral(BLUNO_GLOVE_PLAYER_2_MAC_ADDRESS, "public")
            ...
            self.connPacketReceived = False
            self.connected = False
            print("UNABLE TO CONNECT TO PLAYER 2 GLOVE")
            raise Exception
                ...
                while not self.helloPacketReceived:
                    try:
                        while True:
                            ...
                            #         print("Hello Packet Received for Player 2 Glove")
                            #     elif not self.connPacketReceived:
                            #         self.connPacketReceived = True
                            #         print("Conn Packet Received for Player 2 Glove")
                            #     elif not self.connected:
                            #         print("PLAYER 2 GLOVE READY TO RECEIVED DATA")
                            #         self.connected = True
                        ...
                        print("PLAYER 2 GLOVE DISCONNECTED")
            ...
            if (cHandle==37):
                ...
                if (len(self.byte_array) >= 20):
                    ...
                    if (checksum_value != 0):
                        ...
                        print(GREEN + str(self.index), str(tuple_data1) + END)
...
class BlunoGun:
            ...
            while not (self.helloPacketReceived and self.connPacketReceived):
                try:
                    while True:
                        ...
                        if self.helloPacketReceived and self.connPacketReceived and not gun_queue.empty():
                    ...
                    self.peripheral.disconnect()
                    print(e)
                    print("PLAYER 2 GUN DISCONNECTED")
    ...
    def connectToBLEGun(self):
        ...
        try:
            self.peripheral = Peripheral(BLUNO_GUN_PLAYER_2_MAC_ADDRESS, "public")
            ...
            print("UNABLE TO CONNECT TO PLAYER 2 GUN")
            self.helloPacketReceived = False
            self.connPacketReceived = False
            self.connected = False
            self.connectToBLEGun()
            ...
            if (cHandle==37):
                ...
                if (len(self.byte_array) >= 20):
                    ...
                    if (checksum_value != 0):
                        ...
                        print(tuple_data2[1])
                        if (tuple_data2[1] == ACK_PACKET_ID):

                            if (self.parent.helloPacketReceived == False):
                                print("HELLO PACKET RECEIVED FROM PLAYER 2 GUN")
                                self.parent.helloPacketReceived = True
                                self.parent.connPacketReceived = False
                            elif (self.parent.connPacketReceived == False):
                                print("CONN PACKET RECEIVED FROM PLAYER 2 GUN")
                                self.parent.helloPacketReceived = True
                                self.parent.connPacketReceived = True
...
class BlunoVest:
            ...
            while not (self.helloPacketReceived and self.connPacketReceived):
                try:
                    while True:
                        ...
                        if self.helloPacketReceived and self.connPacketReceived and not vest_queue.empty():
                    ...
                    self.peripheral.disconnect()
                    print(e)
                    print("PLAYER 2 VEST DISCONNECTED")
                    self.helloPacketReceived = False
                    self.connPacketReceived = False
                    self.connected = False
                    self.error_count = 0
    ...
    def connectToBLEVest(self):
        ...
        try:
            self.peripheral = Peripheral(BLUNO_VEST_PLAYER_2_MAC_ADDRESS, "public")
            ...
            print("UNABLE TO CONNECT TO PLAYER 2 VEST")
            self.helloPacketReceived = False
            self.connPacketReceived = False
            self.connected = False
            self.connectToBLEVest()
            ...
            if (cHandle==37):
                ...
                if (len(self.byte_array) >= 20):                    
                    ...
                    if (checksum_value != 0):
                        ...
                        # print(tuple_data3)
                        if (tuple_data3[1] == ACK_PACKET_ID):
                            if (self.parent.helloPacketReceived == False):
                                print("HELLO PACKET RECEIVED FROM PLAYER 2 VEST")
                                self.parent.helloPacketReceived = True
                                self.parent.connPacketReceived = False
                            elif (self.parent.connPacketReceived == False):
                                print("CONN PACKET RECEVIED FROM PLAYER 2 VEST")
                                self.parent.helloPacketReceived = True
                                self.parent.connPacketReceived = True
